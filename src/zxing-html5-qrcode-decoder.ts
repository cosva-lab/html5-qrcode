...
export class ZXingHtml5QrcodeDecoder implements QrcodeDecoder {
  private static formatMap: Map<Html5QrcodeSupportedFormats, any> = new Map([
  ...
  public constructor(requestedFormats: Array<Html5QrcodeSupportedFormats>) {
    if (!ZXing) {
    ...
    this.zxingDecoder = new ZXing.MultiFormatReader();
  ...
  private createZXingFormats(
    ...
    for (let i = 0; i < requestedFormats.length; ++i) {
      if (ZXingHtml5QrcodeDecoder.formatMap.has(requestedFormats[i])) {
        zxingFormats.push(
          ZXingHtml5QrcodeDecoder.formatMap.get(requestedFormats[i])
        );
      } else {
        console.error(
          `${requestedFormats[i]} is not supported by` + 'ZXingHtml5QrcodeShim'
        );
      }
    }
    return zxingFormats;
